...
LastEditTime: 2024-09-23 10:48:55
FilePath: /masa/demo/video_demo_custom1.py
...
def _load_all_categorys(dir:str):
    json_file = os.path.join(dir, "all_categorys.json")
...
def main():
    ...
    device = "cuda:0"
    score_thr = 0.1
    output_dir = "demo_outputs/yoloworld1.mp4"
    
    # load yolo world results
    yolo_det_dir = "/home/frank/data/yoloworld_v2_s_monitor/mods_0920"
    global TYPE_DICT
    TYPE_DICT =  _load_all_categorys(yolo_det_dir)
    ...
    for frame_idx, dets_frame in tqdm(enumerate(dets_all),total=len(dets_all)):
        ...
        if len(result.pred_instances.bboxes) == 0:
    ...
    with Pool(processes=num_cores) as pool:
        frames = pool.starmap(
            visualize_frame, [(score_thr, visualizer, frame, track_result, idx) for idx, (frame, track_result) in
                                enumerate(zip(frames, instances_list))]
        )

    for frame_idx, frame in enumerate(frames):
        video_writer.write(frame[:, :, ::-1])
        file_name = os.path.basename(dets_all[frame_idx].img_path)
        out_img_path = os.path.join("demo_outputs", file_name)
        cv2.imwrite(out_img_path, frame[:, :, ::-1])
